                ...
                INT_32 arg)
{
  ...
  if (psspregs == SSP0)//map real reg ssp0
  {
                ...
                INT_32 max_fifo)
{
  ...
  INT_32 count = 0;
  SSP_DRVDAT_T *sspcfgptr = (SSP_DRVDAT_T *) devid;//引用结构体数据的指针
  UNS_16 *data16 = (UNS_16 *) buffer;//把BUFFER的指针提出来交给变量data16
  UNS_8 *data8 = (UNS_8 *) buffer;

  if (sspcfgptr->init == TRUE)//变量指针引用了真实的芯片内部寄存器的初始位
  {
    while ((max_fifo > 0) &&
           ((sspcfgptr->regptr->sr & SSP_SR_RNE) != 0))//FIFO不空
    {
      tmp1 = sspcfgptr->regptr->data;//读取数据
