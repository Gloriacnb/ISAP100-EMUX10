 ...
 *
 **********************************************************************/
static STATUS mlcnand_read_id(const MLCNAND_DEV_T *dev, /* IN: NAND Device structure */
 ...
 *
 **********************************************************************/
static struct nand_ids * mlcnand_identify_device(const MLCNAND_DEV_T *dev,
 ...
 *
 **********************************************************************/
static void mlcnand_write_addr(const MLCNAND_DEV_T *dev, INT_32 column,
		UNS_32 sector) {
 ...
 *
 **********************************************************************/
static void mlcnand_start_dma(MLCNAND_DEV_T *dev, INT_32 desc_index,
		UNS_32 config) {
 ...
 *
 **********************************************************************/
static STATUS mlcnand_prepare_dma(MLCNAND_DEV_T *dev, UNS_8 *buff, UNS_8 *oob,
		INT_32 read) {
	STATUS status = _NO_ERROR;
	UNS_32 ctrl;
	INT_32 i;
	...
	for (i = 0; i < dev->page_size / 512; i++, dmalst++) {
		...
		dmalst->dma_src = dev->v2p(
				read ? (void *) dev->regs->mlc_buff : &buff[i * 512]);
		dmalst->dma_dest = dev->v2p(
				!read ? (void *) dev->regs->mlc_buff : &buff[i * 512]);
		...
		dmalst->dma_src = dev->v2p(
				read ? (void *) dev->regs->mlc_buff : &oob[i * 16]);
		dmalst->dma_dest = dev->v2p(
				!read ? (void *) dev->regs->mlc_buff : &oob[i * 16]);
		dmalst->next_lli = 0;
 ...
 *
 **********************************************************************/
STATUS mlcnand_read_sector(INT_32 devid, INT_32 sector, UNS_8 *data,
		UNS_8 *spare) {
	int i;
	STATUS stat = _NO_ERROR;
	MLCNAND_DEV_T *dev = mlcnand_dev(devid);
	...
	DMAC_CHAN_IE |
	DMAC_CHAN_FLOW_D_M2M | DMAC_DEST_PERIP(0) | DMAC_SRC_PERIP(0) |
	DMAC_CHAN_ENABLE;
 ...
 *
 **********************************************************************/
STATUS mlcnand_write_sector(INT_32 devid, INT_32 sector, UNS_8 *data,
		UNS_8 *spare) {
	int i;
	STATUS stat = _NO_ERROR;
	MLCNAND_DEV_T *dev = mlcnand_dev(devid);
...
INT_32 mlcnand_open(void *ipbase, INT_32 arg) {
	...
	dev->regs->mlc_lock_pr = MLC_UNLOCK_REG_VALUE;
	dev->regs->mlc_time = (MLC_LOAD_TCEA(15) | MLC_LOAD_TWBTRB(15)
			| MLC_LOAD_TRHZ(15) | MLC_LOAD_TREH(15) | MLC_LOAD_TRP(15)
			| MLC_LOAD_TWH(15) | MLC_LOAD_TWP(15));
...
INT_32 mlcnand_is_block_bad(INT_32 devid, INT_32 block) {
	...
	if (mlcnand_read_sector(devid, block * dev->pages_per_block, 0,
			dev->spare_buff) == _ERROR)
		return -2;
...
INT_32 mlcnand_mark_block_bad(INT_32 devid, INT_32 block) {
	...
	return mlcnand_write_sector(devid, block * dev->pages_per_block, 0,
			dev->spare_buff);
}
