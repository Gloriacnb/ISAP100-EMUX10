...
static UNS_8 errorparse_msg[] = "Syntax is not correct for the command :";
...
static UNS_32 cmd_help_plist[] = { (PARSE_TYPE_STR | PARSE_TYPE_OPT), /* The "help" command */
(PARSE_TYPE_STR | PARSE_TYPE_END) };
static CMD_ROUTE_T cmd_list_head = { (UNS_8 *) "help", cmd_help,
		(UNS_8 *) "S1L help group",
		(UNS_8 *) "help [all, menu, group name, command name]", cmd_help_plist,
		NULL };

/* Starting group list */
static GROUP_LIST_T group_list_head = { (UNS_8 *) "help sys", /* help group */
(UNS_8 *) "Displays help and syntax for a specific command\r\n"
		"Use 'help [all, menu, group name, command name]'", &cmd_list_head,
NULL };
 ...
 *
 **********************************************************************/
BOOL_32 cmd_find_index(UNS_8 *cname, GROUP_LIST_T **pGroup, CMD_ROUTE_T **pCmd) {
 ...
 *
 **********************************************************************/
void cmd_strout_format(int start_offs, UNS_8 *start_str, int next_offs,
		UNS_8 *next_str) {
	int idx1 = 0;
	UNS_8 *space = (UNS_8 *) " ";
 ...
 *
 **********************************************************************/
void cmd_show_group_help(GROUP_LIST_T *pGroup, BOOL_32 show_cmds) {
...
BOOL_32 cmd_parse_fields(CMD_ROUTE_T *pCmd) {
	...
	while ((pfield < fields) && (good == TRUE)) {
		if (pfield == 0) {
			...
			case PARSE_TYPE_DEC:
				good = str_dec_to_val(get_parsed_entry(pfield), &tval);
				break;

			case PARSE_TYPE_HEX:
				good = str_hex_to_val(get_parsed_entry(pfield), &tval);
				break;
 ...
 *
 **********************************************************************/
void cmd_add_new_command(GROUP_LIST_T *pGroup, CMD_ROUTE_T *pCmd) {
