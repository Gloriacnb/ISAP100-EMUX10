...
uint32 rtk_getOUIID(uint8 Phy, uint16 *oui) {
	uint16 regvalue = 0;
	if (oui == NULL || Phy > 6) {
		return FALSE;		  	//杈撳叆鍙傛暟閿欒锛屽垯杩斿洖FAILED
	}
	rtk_getAsicOUIID(Phy, &regvalue);
	*oui = regvalue;
	return TRUE;
}

/*********************************************************************************************************
 ** Function name:   	 rtl8306e_setEtherPhy
 ** Descriptions:         閰嶇疆PHY瀵勫瓨鍣�
 ...
 ** Returned value:       鎴愬姛鎴栧け璐�
 ** Created by:
 ** Created Date:
 **--------------------------------------------------------------------------------------------------------
 ** Modified by:			Wang ying
 ** Modified date:		2011-1-11
 ** Descriptions:
 **********************************************************************************************************/
uint32 rtl8306e_setEtherPhy(uint32 phy, uint32 autoNegotiation,
		uint32 advCapability, uint32 speed, uint32 fullDuplex) {
//    uint32 ctrlReg = 0;
	uint32 autoNegotiation_read, advCapability_read, speed_read,
			fullDuplex_read;
	...
	if (rtl8306e_port_etherPhy_set(phy, autoNegotiation, advCapability, speed,
			fullDuplex) != TRUE) {
		return FALSE;
	}
	if (rtl8306e_port_etherPhy_get(phy, &autoNegotiation_read,
			&advCapability_read, &speed_read, &fullDuplex_read) != TRUE) {
		return FALSE;
	}
// 	if ((autoNegotiation_read != autoNegotiation)
// 			|| (advCapability_read != advCapability) || (speed_read != speed)
// 			|| (fullDuplex_read != fullDuplex)) {
// 		return FALSE;	//灏嗗啓鍏ュ�涓庤鍑哄�杩涜姣旇緝锛岃嫢涓嶇浉鍚屽垯杩斿洖閿欒
// 	}
	return TRUE;
}

/*********************************************************************************************************
 ** Function name:   	 rtl8306e_getPhyLoopback
 ** Descriptions:         璇诲彇PHY鐜洖鐘舵�
 ** input parameters:     phy                 Specify the phy to configure
 ** output parameters:    pEnabled            鐘舵�
 ** Returned value:       鎴愬姛鎴栧け璐�
 ** Created by:
 ** Created Date:
 **--------------------------------------------------------------------------------------------------------
...
uint32 rtl8306e_getPhyLoopback(uint32 phy, uint32 *pEnabled) {
	if (rtl8306e_port_phyLoopback_get(phy, pEnabled) != TRUE) {
		return FALSE;
	}
	return TRUE;
}
...
/*********************************************************************************************************
 ** Function name:   	 rtl8306e_getPhyAutoNegotiationDone
 ** Descriptions:         璇诲彇PHY鑷崗鍟嗙姸鎬�
 ** input parameters:     phy                 Specify the phy to configure
 ** output parameters:    pDone               鐘舵�
 ** Returned value:       鎴愬姛鎴栧け璐�
 ** Created by:
 ** Created Date:
 **--------------------------------------------------------------------------------------------------------
...
uint32 rtl8306e_getPhyAutoNegotiationDone(uint32 phy, uint32 *pDone) {
	if (rtl8306e_port_phyAutoNegotiationDone_get(phy, pDone) != TRUE) {
		return FALSE;
	}
	return TRUE;
}
...
/*********************************************************************************************************
 ** Function name:   	 rtl_VLANEnable_set
 ** Descriptions:         閰嶇疆VLAN 浣胯兘
...
//	rtl8306e_vlan_tagAware_set(enabled) ;
	if (enabled != 0) {
...
int32 rtk_port_phyReg_set(rtk_port_t phy, rtk_port_phy_reg_t reg,
		uint32 regData) {
	uint32 retVal;
...
int32 rtk_port_phyReg_get(rtk_port_t phy, rtk_port_phy_reg_t reg, uint16 *pData) {
	uint32 retVal;
 ...
 */
uint32 rtk_port_phyAutoNegoAbility_set(rtk_port_t port,
	...
	if (pAbility->Half_10 >= RTK_ENABLE_END
			|| pAbility->Full_10 >= RTK_ENABLE_END
			|| pAbility->Half_100 >= RTK_ENABLE_END
			|| pAbility->Full_100 >= RTK_ENABLE_END
			|| pAbility->AutoNegotiation >= RTK_ENABLE_END
			|| pAbility->AsyFC >= RTK_ENABLE_END
	...
	if ((retVal = rtk_port_phyReg_get(port,
			(rtk_port_phy_reg_t) PHY_AN_ADVERTISEMENT_REG, &phyData))
			!= RT_ERR_OK)
		return retVal;

	phyData = (phyData & (~0x0DE0)) | phyEnMsk4;
	if ((retVal = rtk_port_phyReg_set(port,
			(rtk_port_phy_reg_t) PHY_AN_ADVERTISEMENT_REG, phyData))
			!= RT_ERR_OK)
		return retVal;
	...
	if ((retVal = rtk_port_phyReg_get(port,
			(rtk_port_phy_reg_t) PHY_CONTROL_REG, &phyData)) != RT_ERR_OK)
		return retVal;
 ...
 *      (2) In auto-negotiation mode, phy autoNegotiation ability must be enabled
 */
uint32 rtk_port_phyAutoNegoAbility_get(rtk_port_t port,
		rtk_port_phy_ability_t *pAbility) {
	uint16 phyData0;
	uint16 phyData4;
	uint32 retVal;
	...
	/*Control register setting and restart auto*/
	if ((retVal = rtk_port_phyReg_get(port,
			(rtk_port_phy_reg_t) PHY_CONTROL_REG, &phyData0)) != RT_ERR_OK)
		return retVal;

	/*Auto-Negotiation control register setting*/
	if ((retVal = rtk_port_phyReg_get(port,
			(rtk_port_phy_reg_t) PHY_AN_ADVERTISEMENT_REG, &phyData4))
			!= RT_ERR_OK)
		return retVal;
 ...
 */
uint32 rtk_port_phyForceModeAbility_set(rtk_port_t port,
		rtk_port_phy_ability_t *pAbility) {
	uint32 retVal;
	uint16 phyData;
	uint32 phyEnMsk0;
 ...
 *      (2) In force mode, phy autoNegotiation ability must be disabled.
 */
uint32 rtk_port_phyForceModeAbility_get(rtk_port_t port,
		rtk_port_phy_ability_t *pAbility) {
	uint16 phyData0;
	uint16 phyData4;
	uint32 retVal;
 ...
 */
uint32 rtk_port_phyStatus_get(rtk_port_t port,
		rtk_port_linkStatus_t *pLinkStatus, rtk_port_speed_t *pSpeed,
	...
	if (TRUE == linkUp) {
		...
		*pSpeed =
				((phyData0 & (0x1 << 13)) >> 13) ?
						PORT_SPEED_100M : PORT_SPEED_10M;
		*pDuplex =
				((phyData0 & (0x1 << 8)) >> 8) ?
						PORT_FULL_DUPLEX : PORT_HALF_DUPLEX;
	} else {
 ...
 */
uint32 rtk_port_macForceLinkExt0_set(rtk_mode_ext_t mode,
		rtk_port_mac_ability_t *pPortability) {
	uint32 retVal;
	uint16 regVal;

	if (pPortability->speed > 1 || pPortability->duplex > 1
			|| pPortability->txpause > 1 || pPortability->rxpause > 1
			|| pPortability->link > 1)
		return RT_ERR_INPUT;
	...
	if ((retVal = rtl8306e_regbit_set(6, 22, 15, 0, pPortability->link ? 1 : 0))
			!= RT_ERR_OK)
		return retVal;
	...
	if ((pPortability->speed == PORT_SPEED_100M)
		...
		regVal |= 0x30;
	else if ((pPortability->speed == PORT_SPEED_10M)
			&& (pPortability->duplex == PORT_FULL_DUPLEX))
		regVal |= 0x10;
	else if ((pPortability->speed == PORT_SPEED_100M)
	...
	if ((retVal = rtl8306e_regbit_set(6, 24, 12, 0, 1)) != RT_ERR_OK)
		return retVal;
	if ((retVal = rtl8306e_regbit_set(6, 24, 14, 0,
			pPortability->txpause ? 0 : 1)) != RT_ERR_OK)
		return retVal;
	if ((retVal = rtl8306e_regbit_set(6, 24, 13, 0,
			pPortability->rxpause ? 0 : 1)) != RT_ERR_OK)
		return retVal;
 ...
 *      - MODE_EXT_RMII, 
 */
uint32 rtk_port_macForceLinkExt0_get(rtk_mode_ext_t *pMode,
		rtk_port_mac_ability_t *pPortability) {
	uint32 regVal;
	uint32 modesel;
	uint16 regValue = 0;
	...
	rtl8306e_reg_get(6, 26, 3, &regValue);
	modesel = regValue & 0x1F;
	if (0x1 == modesel || 0x2 == modesel || 0x9 == modesel || 0x12 == modesel
			|| 0x10 == modesel)
	...
	rtl8306e_reg_get(6, 22, 0, &regValue);
	if ((regValue & (0x3 << 4)) >> 4 == 0x3) {
		pPortability->speed = 1;
		pPortability->duplex = 1;
	} else if ((regValue & (0x3 << 4)) >> 4 == 0x2) {
		pPortability->speed = 1;
		pPortability->duplex = 0;
	} else if ((regValue & (0x3 << 4)) >> 4 == 0x1) {
		pPortability->speed = 0;
		pPortability->duplex = 1;
	} else if ((regValue & (0x3 << 4)) >> 4 == 0x0) {
		pPortability->speed = 0;
		pPortability->duplex = 0;
	}
	return RT_ERR_OK;
 ...
 *      RT_ERR_FAILED          - FALSE to iset shared meter
 * Note:
 */
...
uint32 rtk_rate_igrBandwidthCtrlRate_set(rtk_port_t port, rtk_rate_t rate,
	...
	if ((retVal = rtl8306e_qos_portRate_set(port, rate, RTL8306_PORT_RX, TRUE))
			!= TRUE)
		return retVal;
...
uint32 rtk_rate_igrBandwidthCtrlRate_get(rtk_port_t port, rtk_rate_t *pRate,
	...
	if ((retVal = rtl8306e_qos_portRate_get(port, pRate, RTL8306_PORT_RX,
			&enabled)) != TRUE)
		return retVal;
...
uint32 rtk_rate_egrBandwidthCtrlRate_set(rtk_port_t port, rtk_rate_t rate,
		rtk_enable_t ifg_include) {
	uint32 retVal;
	...
	if ((retVal = rtl8306e_qos_portRate_set(port, rate, RTL8306_PORT_TX, TRUE))
			!= TRUE)
		return retVal;
...
uint32 rtk_rate_egrBandwidthCtrlRate_get(rtk_port_t port, rtk_rate_t *pRate,
		rtk_enable_t *pIfg_include) {
	uint32 retVal;
	uint32 enabled;
	uint32 disabled;
	...
	if ((retVal = rtl8306e_qos_portRate_get(port, pRate, RTL8306_PORT_TX,
			&enabled)) != TRUE)
		return retVal;
...
uint32 rtk_qos_init(rtk_queue_num_t queueNum) {
	...
	if (rtl8306e_qos_queueNum_set(queueNum) == FALSE)
		return FALSE;
	...
	case 4:
		...
		for (queue = 0; queue < 4; queue++) {
			/*queue threshold*/
			rtl8306e_qos_queFlcThr_set(queue, RTL8306_FCO_QLEN, RTL8306_FCON,
			RTL8306_FCO_SET0, 9);
			rtl8306e_qos_queFlcThr_set(queue, RTL8306_FCO_QLEN, RTL8306_FCOFF,
			RTL8306_FCO_SET0, 5);
			rtl8306e_qos_queFlcThr_set(queue, RTL8306_FCO_DSC, RTL8306_FCON,
			RTL8306_FCO_SET0, 40);
			rtl8306e_qos_queFlcThr_set(queue, RTL8306_FCO_DSC, RTL8306_FCOFF,
			RTL8306_FCO_SET0, 28);
...
uint32 rtk_qos_priSel_set(rtk_priority_select_t *pPriDec) {
	...
	if (NULL == pPriDec)
		return RT_ERR_NULL_POINTER;
	if (pPriDec->port_pri > 5 || pPriDec->dot1q_pri > 5 || pPriDec->acl_pri > 5
			|| pPriDec->dscp_pri > 5 || pPriDec->vid_pri > 5)
	...
	if ((retVal = rtl8306e_qos_priSrcArbit_set(priArbit)) != TRUE)
		return retVal;
...
uint32 rtk_qos_1pPriRemap_set(rtk_pri_t dot1p_pri, rtk_pri_t int_pri) {
	...
	if ((retVal = rtl8306e_qos_1pPriRemap_set(dot1p_pri, int_pri)) != TRUE)
		return retVal;
...
uint32 rtk_qos_1pPriRemap_get(rtk_pri_t dot1p_pri, rtk_pri_t *pInt_pri) {
	...
	if ((retVal = rtl8306e_qos_1pPriRemap_get(dot1p_pri, pInt_pri)) != TRUE)
		return retVal;
 ...
 *          RTL8306_DSCP_AFL2       
 *                 - DSCP for AF PHB Class 2 low drop, 001001
 *          RTL8306_DSCP_AFM2       
...
int32 rtk_qos_dscpPriRemap_set(uint32 dscp, rtk_pri_t int_pri) {
	...
	if ((retVal = rtl8306e_qos_dscpPriRemap_set(dscp, int_pri)) != TRUE) {
		return RT_ERR_FAILED;
	}
	return retVal;
}
...
int32 rtk_qos_dscpPriRemap_get(uint32 dscp, rtk_pri_t *pInt_pri) {
	...
	if ((retVal = rtl8306e_qos_dscpPriRemap_get(dscp, pInt_pri)) != TRUE)
		return RT_ERR_FAILED;
...
uint32 rtk_qos_portPri_set(rtk_port_t port, rtk_pri_t int_pri) {
	...
	if ((retVal = rtl8306e_qos_portPri_set(port, int_pri)) != TRUE)
		return retVal;
...
uint32 rtk_qos_portPri_get(rtk_port_t port, rtk_pri_t *pInt_pri) {
	...
	if ((retVal = rtl8306e_qos_portPri_get(port, pInt_pri)) != TRUE)
		return retVal;
...
int32 rtk_qos_priMap_set(rtk_queue_num_t queue_num,
	...
	if ((retVal = rtl8306e_qos_queueNum_set(queue_num)) != TRUE)
		return RT_ERR_FAILED;

	for (pri = RTL8306_PRIO0; pri <= RTL8306_PRIO3; pri++) {
		if (pPri2qid->pri2queue[pri] > (queue_num - 1))
			return RT_ERR_QUEUE_ID;
		if ((retVal = rtl8306e_qos_priToQueMap_set(pri,
				pPri2qid->pri2queue[pri])) != TRUE)
			return RT_ERR_FAILED;
	}
...
int32 rtk_qos_priMap_get(rtk_queue_num_t queue_num,
		rtk_qos_pri2queue_t *pPri2qid) {
	uint32 retVal;
	uint32 pri;
	...
	for (pri = RTL8306_PRIO0; pri <= RTL8306_PRIO3; pri++) {
		if ((retVal = rtl8306e_qos_priToQueMap_get(pri,
				&(pPri2qid->pri2queue[pri]))) != TRUE)
			return RT_ERR_FAILED;
	}
	return retVal;
...
int32 rtk_qos_1pRemarkEnable_set(rtk_port_t port, rtk_enable_t enable) {
	...
	if ((retVal = rtl8306e_qos_1pRemarkEnable_set(port, enable)) != TRUE)
		return RT_ERR_FAILED;
...
int32 rtk_qos_1pRemarkEnable_get(rtk_port_t port, rtk_enable_t *pEnable) {
	...
	if (NULL == pEnable)
		return RT_ERR_NULL_POINTER;
	if ((retVal = rtl8306e_qos_1pRemarkEnable_get(port, (uint32 *) pEnable))
			!= TRUE)
		return RT_ERR_FAILED;
	return retVal;
}
...
int32 rtk_qos_1pRemark_set(rtk_pri_t int_pri, rtk_pri_t dot1p_pri) {
	...
	if ((retVal = rtl8306e_qos_1pRemark_set(int_pri, dot1p_pri)) != TRUE)
		return RT_ERR_FAILED;
...
int32 rtk_qos_1pRemark_get(rtk_pri_t int_pri, rtk_pri_t *pDot1p_pri) {
	...
	if ((retVal = rtl8306e_qos_1pRemark_get(int_pri, pDot1p_pri)) != TRUE)
		return RT_ERR_FAILED;
 ...
 *               - MCAST_ACTION_DROP
 */
int32 rtk_trap_unknownMcastPktAction_set(rtk_port_t port, rtk_mcast_type_t type,
	...
	if (MCAST_IPV4 == type) {
		retVal = rtl8306e_trap_unknownIPMcastPktAction_set(
				RTL8306_IPV4_MULTICAST,
				(MCAST_ACTION_FORWARD == mcast_action) ?
						RTL8306_ACT_PERMIT : RTL8306_ACT_DROP);
		if (retVal != TRUE)
			return RT_ERR_FAILED;
 ...
 *               - MCAST_ACTION_DROP
 */
int32 rtk_trap_unknownMcastPktAction_get(rtk_port_t port, rtk_mcast_type_t type,
		rtk_trap_mcast_action_t *pMcast_action) {
	uint32 retVal;
	uint32 action;
	...
	if (MCAST_IPV4 == type) {
		retVal = rtl8306e_trap_unknownIPMcastPktAction_get(
				RTL8306_IPV4_MULTICAST, &action);
		if (retVal != TRUE)
			return RT_ERR_FAILED;
		*pMcast_action =
				(action == RTL8306_ACT_PERMIT) ?
						MCAST_ACTION_FORWARD : MCAST_ACTION_DROP;
	} else if (MCAST_IPV6 == type) {
		retVal = rtl8306e_trap_unknownIPMcastPktAction_get(
				RTL8306_IPV6_MULTICAST, &action);
		if (retVal != TRUE)
			return RT_ERR_FAILED;
		*pMcast_action =
				(action == RTL8306_ACT_PERMIT) ?
 ...
 */
int32 rtk_trap_igmpCtrlPktAction_set(rtk_igmp_type_t type,
	...
	action =
			(igmp_action == IGMP_ACTION_FORWARD) ?
					RTL8306_ACT_PERMIT : RTL8306_ACT_TRAPCPU;
 ...
 *          - IGMP_ACTION_TRAP2CPU
 */
int32 rtk_trap_igmpCtrlPktAction_get(rtk_igmp_type_t type,
		rtk_trap_igmp_action_t *pIgmp_action) {
	uint32 retVal;
	uint32 igmp_type, action;
	...
	*pIgmp_action =
			(action == RTL8306_ACT_PERMIT) ?
					IGMP_ACTION_FORWARD : IGMP_ACTION_TRAP2CPU;
...
uint32 rtk_vlan_init(void) {
	...
	uint32 port;
	for (i = 0; i < 16; i++)
		rtl8306e_vlan_entry_set(i, 0, 0, 0); /*clear vlan table*/
 ...
 *     so fid is no useage.
 */
uint32 rtk_vlan_set(rtk_vlan_t vid, rtk_portmask_t mbrmsk,
		rtk_portmask_t untagmsk, uint32 fid) {
	uint16 vid_val, mbrmsk_val;
	uint16 untagmsk_val;
	...
	hit_index = 16;
	for (i = 15; i >= 0; i--) {
		rtl8306e_vlan_entry_get((uint32) i, &vid_val, &mbrmsk_val,
				&untagmsk_val);
		if (0 == vid_val) {
			index = (uint32) i;
			fullflag = FALSE;
	...
	if (fullflag)
		...
		if (hit_flag)
			rtl8306e_vlan_entry_set(hit_index, vid_val, mbrmsk_val,
					untagmsk_val);
		else
...
uint32 rtk_vlan_get(rtk_vlan_t vid, rtk_portmask_t *pMbrmsk,
		rtk_portmask_t *pUntagmsk, uint32 *pFid) {
	...
	hit_flag = FALSE;
	for (i = 15; i >= 0; i--) {
		rtl8306e_vlan_entry_get((rtk_vlan_t) i, &vid_val, &mbrmsk_val,
				&untagmsk_val);
		if (vid_val == vid) {
			hit_flag = TRUE;
 ...
 * 
 */
uint32 rtk_vlan_destroy(rtk_vlan_t vid) {
	uint16 vid_val, mbrmsk_val, untagmsk_val;
	uint32 hit_flag;
	int32 i;
	...
	for (i = 15; i >= 0; i--) {
		...
		if (vid_val == vid) {
			...
			mbrmsk_val = 0;
			untagmsk_val = 0;
			rtl8306e_vlan_entry_set((uint32) i, vid_val, mbrmsk_val,
					untagmsk_val);
			return RT_ERR_OK;
		}
	}
 ...
 */
uint32 rtk_vlan_portPvid_set(rtk_port_t port, rtk_vlan_t pvid,
		rtk_pri_t priority) {
	uint16 vid_val, mbrmsk_val, untagmsk_val;
	uint32 hit_flag;
	uint16 regVal;
...
int32 rtk_vlan_portPvid_get(rtk_port_t port, rtk_vlan_t *pPvid,
		rtk_pri_t *pPriority) {
	uint32 retVal;
	uint16 vid_val, mbrmsk_val, untagmsk_val;
	uint16 index;
	...
	retVal = rtl8306e_vlan_entry_get(index, &vid_val, &mbrmsk_val,
			&untagmsk_val);
	if (retVal != TRUE)
		return RT_ERR_FAILED;
	*pPvid = vid_val;
 ...
 *      to VLAN member ports, ASIC will drop received frame if VLAN ingress function is enabled.
 */
int32 rtk_vlan_portIgrFilterEnable_get(rtk_port_t port,
		rtk_enable_t *pIgr_filter) {
	uint32 retVal;
	uint32 enabled;
 ...
 */
uint32 rtk_vlan_portAcceptFrameType_set(rtk_port_t port,
...
uint32 rtk_vlan_portAcceptFrameType_get(rtk_port_t port,
 ...
 *      is UNI port
 */
uint32 rtk_vlan_vidTranslationEnable_set(rtk_enable_t enable,
	...
	if (enable) {
		for (port = 0; port < 6; port++)
			rtl8306e_svlan_otagSrc_set(port, RTL8306E_VIDSRC_NVID,
					RTL8306E_PRISRC_1PRMK);
	}
...
uint32 rtk_vlan_vidTranslationEnable_get(rtk_enable_t *pEnable,
 ...
 */
uint32 rtk_vlan_tagSrc_set(rtk_port_t port, rtk_vidSrc_t vidSrc,
		rtk_priSrc_t priSrc) {
	uint32 retVal;
 ...
 *                RTL8306E_PRISRC_1PRMK - 1p remarking priority
 */
uint32 rtk_vlan_tagSrc_get(rtk_port_t port, rtk_vidSrc_t *pVidSrc,
		rtk_priSrc_t *pPriSrc) {
	uint32 retVal;
 ...
 */
uint32 rtk_stp_mstpState_set(rtk_stp_msti_id_t msti, rtk_port_t port,
 ...
 *          - STP_STATE_FORWARDING
 */
uint32 rtk_stp_mstpState_get(rtk_stp_msti_id_t msti, rtk_port_t port,
...
int32 rtk_l2_addr_add(rtk_mac_t *pMac, rtk_l2_ucastAddr_t *pL2_data) {
	...
	retVal = rtl8306e_l2_unicastMac_add(pMac->octet, RTL8306_LUT_AGE300,
			pL2_data->is_static, pL2_data->auth, pL2_data->port, &entryAddr);
...
int32 rtk_l2_addr_get(rtk_mac_t *pMac, uint32 fid, rtk_l2_ucastAddr_t *pL2_data) {
	...
	retVal = rtl8306e_l2_mac_get(pMac->octet, &isStatic, &isAuth, &port,
			&entryaddr);
...
int32 rtk_l2_mcastAddr_add(rtk_mac_t *pMac, uint32 fid, rtk_portmask_t portmask) {
	...
	retVal = rtl8306e_l2_multicastMac_add(pMac->octet, TRUE, portmask.bits[0],
			&entryAddr);
	if (RTL8306_LUT_FULL == retVal) {
		return RT_ERR_L2_INDEXTBL_FULL;
	} else if (retVal != TRUE) {
...
int32 rtk_l2_mcastAddr_get(rtk_mac_t *pMac, uint32 fid,
	...
	retVal = rtl8306e_l2_mac_get(pMac->octet, &isStatic, &isAuth, &portmask,
			&entryaddr);
	if (RTL8306_LUT_NOTEXIST == retVal) {
		return RT_ERR_L2_ENTRY_NOTFOUND;
	} else if (retVal != TRUE) {
...
uint32 rtk_l2_limitLearningCnt_set(rtk_port_t port, rtk_mac_cnt_t mac_cnt) {
	...
	if ((retVal = rtl8306e_l2_portMacLimit_set(port, enabled, mac_cnt))
			!= RT_ERR_OK)
		return retVal;
...
uint32 rtk_l2_limitLearningCnt_get(rtk_port_t port, rtk_mac_cnt_t *pMac_cnt) {
	...
	if ((retVal = rtl8306e_l2_portMacLimit_get(port, &enabled, pMac_cnt))
			!= RT_ERR_OK)
		return retVal;
 ...
 */
uint32 rtk_l2_limitLearningCntAction_set(rtk_port_t port,
 ...
 *      (2)The action is global, so the port must be set as RTK_WHOLE_SYSTEM
 */
int32 rtk_l2_limitLearningCntAction_get(rtk_port_t port,
...
uint32 rtk_mirror_portBased_set(rtk_port_t mirroring_port,
 ...
 *      The API is to set mirror function of source port and mirror port. 
 */
uint32 rtk_mirror_portBased_get(rtk_port_t *pMirroring_port,
		rtk_portmask_t *pMirrored_rx_portmask,
		rtk_portmask_t *pMirrored_tx_portmask) {
	uint32 retVal;
 ...
 */
uint32 rtk_dot1x_unauthPacketOper_set(rtk_port_t port,
	...
	if (DOT1X_ACTION_DROP == unauth_action) {
		retVal = rtl8306e_trap_abnormalPktAction_set(RTL8306_DOT1XUNAUTH,
				RTL8306_ACT_DROP);
 ...
 *          - DOT1X_ACTION_TRAP2CPU
 */
int32 rtk_dot1x_unauthPacketOper_get(rtk_port_t port,
...
int32 rtk_dot1x_portBasedEnable_set(rtk_port_t port, rtk_enable_t enable) {
	...
	retVal = rtl8306e_dot1x_portBased_set(port, (uint32) enable, isAuth,
			direction);
	if (retVal != TRUE)
		return RT_ERR_FAILED;
...
int32 rtk_dot1x_portBasedAuthStatus_set(rtk_port_t port,
	...
	retVal = rtl8306e_dot1x_portBased_set(port, enDot1x, (uint32) port_auth,
			direction);
	if (retVal != TRUE)
		return RT_ERR_FAILED;
...
int32 rtk_dot1x_portBasedDirection_set(rtk_port_t port,
	...
	retVal = rtl8306e_dot1x_portBased_set(port, enDot1x, isAuth,
...
int32 rtk_dot1x_macBasedAuthMac_add(rtk_port_t port, rtk_mac_t *pAuth_mac,
		uint32 fid) {
	int32 i;
	uint8 macAddr[6];
	...
	for (i = 3; i >= 0; i--) {
		...
		if (rtl8306e_l2_unicastEntry_get(macAddr, entryaddr, &age, &isStatic,
				&isAuth, &srcPort) != TRUE) {
			return RT_ERR_FAILED;
		} else if ((pAuth_mac->octet[0] == macAddr[0])
				&& (pAuth_mac->octet[1] == macAddr[1])
				&& (pAuth_mac->octet[2] == macAddr[2])
				&& (pAuth_mac->octet[3] == macAddr[3])
				&& (pAuth_mac->octet[4] == macAddr[4])
				&& (pAuth_mac->octet[5] == macAddr[5])) {
			if (srcPort != port)
				return RT_ERR_DOT1X_MAC_PORT_MISMATCH;
			if (rtl8306e_l2_unicastEntry_set(pAuth_mac->octet, (uint32) i, age,
					isStatic, TRUE, port) != TRUE)
				return RT_ERR_FAILED;
			isHit = TRUE;
			return RT_ERR_OK;
		}
 ...
 */
int32 rtk_dot1x_macBasedAuthMac_del(rtk_port_t port, rtk_mac_t *pAuth_mac,
		uint32 fid) {
	int32 i;
	uint8 macAddr[6];
	uint32 index, entryaddr;
 ...
 *    port will be classified to the specified VLAN and assigned to the specified priority.
 */
uint32 rtk_svlan_portPvid_set(rtk_port_t port, rtk_vlan_t pvid,
		rtk_pri_t priority, rtk_dei_t dei) {
	uint16 regval;
 ...
 *    port will be classified to the specified VLAN and assigned to the specified priority.
 */
uint32 rtk_svlan_portPvid_get(rtk_port_t port, rtk_vlan_t *pPvid,
		rtk_pri_t* pPriority, rtk_dei_t *pDei) {
	uint16 regval;
...
int32 rtk_filter_igrAcl_init(void) {
	...
	for (entaddr = 0; entaddr < RTL8306_ACL_ENTRYNUM; entaddr++) {
		if (rtl8306e_acl_entry_set(entaddr, RTL8306_ACL_INVALID,
				RTL8306_ACT_DROP, RTL8306_ACL_ETHER, 0, 0) == FALSE)
			return RT_ERR_FAILED;
	}
...
int32 rtk_filter_igrAcl_rule_add(rtk_filter_rule_t *pRule) {
	 ...
	 */
	for (entaddr = 0; entaddr < RTL8306_ACL_ENTRYNUM; entaddr++) {
		if (rtl8306e_acl_entry_get(entaddr, &port, &act, &pro, &val,
				&pri) != TRUE)
			return RT_ERR_FAILED;

		if ((pRule->phyport == port) && (pRule->protocol == pro)
				&& (pRule->data == val)) {
			if (rtl8306e_acl_entry_set(entaddr, pRule->phyport, pRule->action,
					pRule->protocol, pRule->data, pRule->priority) != TRUE)
				return RT_ERR_FAILED;
			else
				return RT_ERR_OK;
		}
...
int32 rtk_filter_igrAcl_rule_del(rtk_filter_rule_t *pRule) {
	...
	for (entaddr = 0; entaddr < RTL8306_ACL_ENTRYNUM; entaddr++) {
		...
		if ((port == pRule->phyport) && (pro == pRule->protocol)
				&& (val == pRule->data)) {
			if (rtl8306e_acl_entry_set(entaddr, RTL8306_ACL_INVALID,
					RTL8306_ACT_DROP, RTL8306_ACL_ETHER, 0, 0) != TRUE)
				return RT_ERR_FAILED;
			isHit = TRUE;
			break;
		}
...
uint32 RTL8306EWRRConfig(rtl8306e_qosSchPara_t sch_para) {
	uint32 i = 0;
	for (i = 0; i < 6; i++) {
		rtl8306e_qos_portSchedulingMode_set(i, RTL8306_QOS_SET1, 0x0);//4,1,0x0
	}
//	rtl8306e_qos_portSchedulingMode_set(5, RTL8306_QOS_SET1, 0x0);	//4,1,0x0
	if (rtl8306e_qos_schedulingPara_set(RTL8306_QOS_SET1, sch_para) != TRUE) {
		return FALSE;
	}
	return TRUE;
}
...
// 	for(i = 0;i< 200000;i++);	   //delay 5ms	100000

// 	FIO0MASK &= ~0x01;
// 	FIO0SET |= 0x01;
// 	for( i = 0;i< 2000000;i++);	   //delay 5ms	200000
// 	return;
// }
